...
import { buildAddCompoundingLiquidityTx } from "@/lib/flash-extra";
...
export async function POST(req: Request) {
  ...
  try {
    const transaction = await buildAddCompoundingLiquidityTx(body);
    return Response.json({ transaction });
  } catch (err) {
    console.error("[flash/build-add-compounding]", err);
